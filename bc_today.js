...
*/
// Not said to be done in the book, but put what they said to do in function, the called it and made it repeat so that the time would update.
getDay();
setInterval('getDay()', 1000);

function getDay() {
    // Sets up the variiables, gets today's date, then extracts what day of the week it is and the date string, then created html to insert what day it was.
    var thisDate = new Date();
    var dateString = thisDate.toLocaleDateString();
    var thisDay = thisDate.getDay();
    var dateHTML = `<h1>Today at the Union</h1> <h2 id="dateString">${dateString}</h2>`;
    // Calls the getEvent with the parameter of thisDay, then applies it to the document
